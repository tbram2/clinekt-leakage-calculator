...
(function () {
  ...
  /* 300 shortest links, precomputed */
  var LINKS = (function () {
    var all = [], a, b;
    for (a = 0; a < N; a++) for (b = a + 1; b < N; b++) {
      var dx = PTS[a][0] - PTS[b][0], dy = PTS[a][1] - PTS[b][1], dz = PTS[a][2] - PTS[b][2];
      var q = dx * dx + dy * dy + dz * dz; if (q < 0.09) all.push([a, b, q]);
    }
    all.sort(function (u, v) { return u[2] - v[2]; }); return all.slice(0, 300);
  })();

  function start(cv) {
    if (cv.__ckbrain) return; cv.__ckbrain = 1;
    var ctx = cv.getContext('2d'); if (!ctx) return;
    var labels = cv.getAttribute('data-labels') !== 'off';
    var W = 0, H = 0, R = 0, CX = 0, CY = 0, ro, io;
    var nodes = [], pulses = [], rips = [], proj = new Float32Array(N * 3);
    var t = 0, last = 0, due = 0.6, raf = 0, seen = true;

    function size() {
      var b = cv.getBoundingClientRect(); if (b.width < 2 || b.height < 2) return false;
    ...
    function spawn() {
      ...
      var ex = CX + dx / dl * R * 1.02, ey = CY + dy / dl * R * 1.02, toAgent = rnd() >= 0.35;
      var x0 = toAgent ? ex : nd.x, y0 = toAgent ? ey : nd.y, x2 = toAgent ? nd.x : ex, y2 = toAgent ? nd.y : ey;
    ...
    function step(dt) {
      if (t >= due) { due = t + 1 + rnd() * 0.2; if (pulses.length < 3) spawn(); }
      for (var i = pulses.length - 1; i >= 0; i--) {
        var u = pulses[i]; u.p += dt / u.dur;
        if (u.p >= 1) { rips.push({ x: u.x2, y: u.y2, c: u.c, a: 0 }); pulses.splice(i, 1); }
      }
      for (i = rips.length - 1; i >= 0; i--) if ((rips[i].a += dt / 0.55) >= 1) rips.splice(i, 1);
    }

    function draw() {
      ctx.clearRect(0, 0, W, H);
      var i, k, g = ctx.createRadialGradient(CX, CY, 0, CX, CY, R * 1.5);
      /* core glow */
      g.addColorStop(0, 'rgba(0,113,227,0.20)'); g.addColorStop(0.55, 'rgba(0,113,227,0.07)'); g.addColorStop(1, 'rgba(0,113,227,0)');
      ctx.fillStyle = g; ctx.beginPath(); ctx.arc(CX, CY, R * 1.5, 0, TAU); ctx.fill();

      /* rotate + project */
      var ry = t * TAU / 34, rx = 0.12 * Math.sin(t * 0.5);
      var cA = Math.cos(ry), sA = Math.sin(ry), cB = Math.cos(rx), sB = Math.sin(rx);
      for (i = 0; i < N; i++) {
        var p = PTS[i], x = p[0] * cA + p[2] * sA, z = p[2] * cA - p[0] * sA;
        var yv = p[1] * cB - z * sB; z = p[1] * sB + z * cB;
        var s = R * CAM / (CAM - z);
        proj[i * 3] = CX + x * s; proj[i * 3 + 1] = CY + yv * s;
        proj[i * 3 + 2] = (z + 1) / 2; /* depth 0..1 */
      }
      /* links */
      ctx.lineWidth = 1;
      for (i = 0; i < LINKS.length; i++) {
        var a3 = LINKS[i][0] * 3, b3 = LINKS[i][1] * 3, d = (proj[a3 + 2] + proj[b3 + 2]) / 2;
      ...
      for (i = 0; i < 4; i++) {
        ...
        if (labels) {
          ctx.fillStyle = 'rgba(255,255,255,0.78)'; ctx.font = '11px Inter,sans-serif'; ctx.textAlign = 'center';
          ctx.fillText(nd.l, nd.x, Math.min(nd.y + 24, H - 5));
        }
      }
      /* pulses */
      for (i = 0; i < pulses.length; i++) {
        var u = pulses[i], e = u.p * u.p * (3 - 2 * u.p); ctx.fillStyle = u.c;
        for (k = 4; k >= 0; k--) {
          var te = e - k * 0.05; if (te < 0) continue;
          var mt = 1 - te; ctx.globalAlpha = (1 - k / 5) * (k ? 0.5 : 1) * Math.min(1, u.p * 6);
          ctx.beginPath(); ctx.arc(mt * mt * u.x0 + 2 * mt * te * u.x1 + te * te * u.x2,
            mt * mt * u.y0 + 2 * mt * te * u.y1 + te * te * u.y2, 2.5 - k * 0.35, 0, TAU); ctx.fill();
        }
      }
      /* rips */
      ctx.lineWidth = 1.5;
      for (i = 0; i < rips.length; i++) {
        var rp = rips[i]; ctx.globalAlpha = 0.7 * (1 - rp.a); ctx.strokeStyle = rp.c;
    ...
    function loop(now) {
      if (gone() || !seen || document.hidden) { raf = 0; return; }
      var dt = last ? Math.min((now - last) / 1000, 0.1) : 0.016;
      last = now; t += dt; step(dt); draw();
      raf = requestAnimationFrame(loop);
    }
    function play() { if (reduced || raf || !seen || document.hidden || !W) return; last = 0; raf = requestAnimationFrame(loop); }
    function halt() { if (raf) { cancelAnimationFrame(raf); raf = 0; } }
    function onVis() { if (!gone()) { if (document.hidden) halt(); else play(); } }
    /* teardown when canvas leaves the DOM; scan() revives on re-attach */
    function gone() {
      if (cv.isConnected !== false) return false;
      halt(); if (ro) ro.disconnect(); if (io) io.disconnect();
      document.removeEventListener('visibilitychange', onVis); cv.__ckbrain = 0; return true;
    }

    if (window.ResizeObserver)
      (ro = new ResizeObserver(function () { if (!gone() && size()) { draw(); play(); } })).observe(cv);
    if (!reduced) {
      if (window.IntersectionObserver)
        (io = new IntersectionObserver(function (es) {
          if (gone()) return;
          seen = es[es.length - 1].isIntersecting; if (seen) play(); else halt();
        })).observe(cv);
      document.addEventListener('visibilitychange', onVis);
    }
    if (size()) { draw(); play(); } /* reduced: one static frame */
  }

  function scan(root) {
    var list = (root && root.querySelectorAll ? root : document).querySelectorAll('canvas.ckbrain,canvas[data-ckbrain]');
